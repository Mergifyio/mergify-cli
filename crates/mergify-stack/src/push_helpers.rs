...
/// whose `change.pull` is set or whose action is `Create` —
/// i.e. anything that has (or will have) a PR. `SkipMerged`
/// entries that still carry a `pull` qualify and become the
/// anchor; the orchestrator walks past them when resolving the
/// `Depends-On:` header so the rendered chain points at the
/// next live PR. Locked-in to match Python (`if change.pull is
/// not None or action == "create"`).
#[must_use]
pub fn build_change_tasks(local_changes: &[LocalChange]) -> Vec<TaskMeta> {
    ...
    for (i, change) in local_changes.iter().enumerate() {
        ...
        // the next change. Matches Python's `if change.pull is
        // not None or action == "create"` — `SkipMerged` with a
        // pull *does* qualify; the orchestrator walks past it
        // when rendering `Depends-On:` so the header still
        // points at the next live PR, not the merged one.
...
mod tests {
    ...
    #[test]
    fn build_skip_merged_with_pull_anchors_next_task_dependency() {
