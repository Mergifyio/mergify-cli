...
//!
//! Mirrors the Python CLI's `_relative_time` helper: an ISO-8601 /
//! RFC-3339 timestamp becomes a short delta like `5m ago` or
//! `~2h`. The granularity is intentionally coarse — seconds /
//! minutes / hours / days, single component — because these
//! numbers show up in dense table layouts where exact fidelity
//! adds noise without information.
//!
//! Unparseable input returns an empty string. Callers treat that
//! as "skip this column" so a single malformed timestamp doesn't
