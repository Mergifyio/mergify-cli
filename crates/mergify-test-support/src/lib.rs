...
impl Captured {
    ...
    pub fn stdout(&self) -> String {
        String::from_utf8(self.stdout.lock().unwrap().clone())
            .expect("captured stdout must be UTF-8")
    }
    ...
    pub fn stderr(&self) -> String {
        String::from_utf8(self.stderr.lock().unwrap().clone())
            .expect("captured stderr must be UTF-8")
    }
