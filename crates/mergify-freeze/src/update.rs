...
pub struct UpdateOptions<'a> {
    ...
    /// `None` omits the key on the wire (PATCH semantics: don't
    /// touch the stored list). `Some(slice)` sends `slice` as the
    /// new value — including `Some(&[])`, which clears the list on
    /// the server side. The CLI boundary always maps "user didn't
    /// pass `-c`" to `None` and "user passed `-c` at least once"
    /// to `Some(non-empty)`, so callers going through
    /// `mergify freeze update` never observe `Some(&[])`; that
    /// shape is reserved for direct programmatic use of this API.
