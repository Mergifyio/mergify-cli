...
/// Decide whether to proceed with a destructive queue-pause.
///
/// `is_tty` is taken as a parameter (rather than read from
/// `std::io::stdin()` here) so unit tests can exercise the
/// non-TTY branch deterministically — `cargo test` happens to run
/// without a terminal stdin most of the time, but that's not
/// something we want to depend on, especially when developers run
/// `cargo test` from inside an interactive shell.
fn confirm(skip: bool, is_tty: bool, repository: &str) -> Result<(), CliError> {
    if skip {
        return Ok(());
    }
    if !is_tty {
...
mod tests {
    ...
    fn confirm_refuses_without_yes_when_non_tty() {
        // Pass `is_tty = false` explicitly so the test exercises
        // the non-TTY branch without depending on the runtime
        // shape of `cargo test`'s stdin (which can be a TTY when
        // run from an interactive shell).
        let err = confirm(false, false, "owner/repo").unwrap_err();
        assert!(
    ...
    fn confirm_skips_when_yes_i_am_sure_is_set() {
        // `skip = true` must bypass the TTY check — it's the
        // contract of `--yes-i-am-sure`, including in CI where
        // stdin isn't a terminal. The `is_tty` value passed here
        // is irrelevant.
        confirm(true, false, "owner/repo").unwrap();
    }
