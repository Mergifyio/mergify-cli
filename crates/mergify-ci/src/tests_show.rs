//! `mergify tests show` — resolve test identities by name and
//! fetch full health/metrics details for each match.
//!
//! Wraps two endpoints from the Mergify CI Insights API:
//!
//! - `GET /v1/ci/{owner}/repositories/{repository_name}/search/tests`
//!   — resolves test identities by name (glob-aware) on the default
//!   branch. Search parameters travel as query parameters; repeatable
//!   filters (`test_name`, `pipeline_name`, …) appear once per value.
//! - `GET /v1/ci/{owner}/repositories/{repository_name}/tests/{test_id}`
//!   — returns the full health/metrics payload for one identity.
//!
//! The search endpoint is natively a batch API: callers pass one or
//! more `<NAME>` positionals (each may include `*` and `?` glob
//! patterns) and the command renders one block per match.
//!
//! Exit code reflects the worst health observed across all results
//! (max severity wins):
//!
//! - `Success` (0) — every test is `healthy` or unknown, or the
//!   search returned no matches at all.
//! - `GenericError` (1) — at least one test is `flaky`.
//! - `MergifyApiError` (6) — at least one test is `broken`.
//!
//! `2` is intentionally not used here: the CLI-wide contract reserves
//! it for clap argument errors.
...
pub async fn run(
    ...
    let details = fetch_all_details(&client, owner, repo, &identities).await?;
...
/// Assemble the `(key, value)` pairs the search endpoint expects.
///
/// The endpoint takes filters as repeated query parameters (one
/// `test_name=…` pair per name, etc.). Optional filters with no
/// values are omitted so the server falls back to its own defaults.
...
async fn fetch_all_details(
    ...
    stream::iter(identities.iter().map(|id| {
        let path = format!("/v1/ci/{owner}/repositories/{repo}/tests/{}", id.test_id);
        async move { client.get::<TestDetails>(&path).await }
    }))
    .buffered(DETAILS_FANOUT)
    .try_collect()
    .await
}
...
fn render_one(w: &mut dyn Write, row: &TestRow) -> io::Result<()> {
    let d = &row.details;
    writeln!(w, "{}", d.test_name)?;
    writeln!(w, "  test_id:        {}", d.test_id)?;
    writeln!(
        w,
        "  pipeline:       {} › job: {}",
        row.pipeline_name, row.job_name,
    )?;
    if let Some(language) = &d.test_programming_language {
        writeln!(w, "  language:       {language}")?;
    }
    if let Some(framework) = &d.test_framework {
        match &d.test_framework_version {
            Some(version) => writeln!(w, "  framework:      {framework} {version}")?,
            None => writeln!(w, "  framework:      {framework}")?,
        }
    }
    if let Some(filepath) = &d.test_filepath {
        writeln!(w, "  file:           {filepath}")?;
    }
    if let Some(function) = &d.test_function_name {
        writeln!(w, "  function:       {function}")?;
    }
    writeln!(w, "  health:         {}", health_label(d.health_status))?;
    writeln!(
        w,
        "  last result:    {}",
        conclusion_label(d.last_conclusion)
    )?;
    writeln!(w, "  success ratio:  {:.1}%", d.success_ratio * 100.0)?;
    writeln!(w, "  failure ratio:  {:.1}%", d.failure_ratio * 100.0)?;
    if d.flaky_detection_enabled {
        writeln!(w, "  flakiness:      {:.1}%", d.flakiness_ratio * 100.0)?;
    }
    if let Some(ts) = d.last_success_at {
        writeln!(w, "  last success:   {}", format_timestamp(ts))?;
    }
    if let Some(ts) = d.last_failure_at {
        writeln!(w, "  last failure:   {}", format_timestamp(ts))?;
    }
    if let Some(ts) = d.first_failure_at {
        writeln!(w, "  first failure:  {}", format_timestamp(ts))?;
        let commit = d.first_failure_commit.as_deref();
        if let Some(pull) = &d.first_failure_pull {
            let prefix = commit
...
mod tests {
    ...
    async fn single_match_fetches_details_and_renders_in_human_mode() {
        ...
        // `flaky_detection_enabled = false` should hide the flakiness
        // line entirely — no placeholder.
        assert!(
            !out.contains("flakiness"),
            "flakiness line must be hidden, got {out:?}"
        );
        assert!(
    ...
    async fn batch_preserves_search_order_even_when_responses_arrive_out_of_order() {
        ...
        // Different per-response delays so completion order differs
        // from search order — verifies the renderer preserves search
    ...
    async fn unknown_health_status_does_not_promote_severity() {
        ...
        // Renderer must show `— unknown`, not crash on the unknown
        // value — verify by switching to human mode.
        let mut cap = captured(OutputMode::Human);
        run(options(&api_url, &names), &mut cap.output)
            .await
            .unwrap();
    ...
    async fn search_query_includes_names_and_filters_and_omits_absent_ones() {
        ...
        // Collect (key, value) pairs so the assertion captures both
        // names and order without depending on percent-encoding rules.
