...
pub struct UploadMetadata {
    ...
    /// else defaults to `false`. Pass an empty set when the
    /// quarantine check was skipped (no failures) or failed
    /// (network/API error) — the spans then upload with every
...
mod tests {
    ...
    fn sample_parsed() -> ParseResult {
        ParseResult {
            suite_names: vec!["pytest".to_string()],
            cases: vec![
                TestCase {
