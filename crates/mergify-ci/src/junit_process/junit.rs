    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Text(e)) => {
                ...
                // `GeneralRef` event below. So plain `decode()` is
                // correct here; entity resolution happens in the
                // `GeneralRef` arm.
                let s = e.decode().map_err(|err| InvalidJunitXml {
...
impl ParserState {
    ...
    fn finalize(self) -> Result<ParseResult, InvalidJunitXml> {
        ...
        Ok(ParseResult { cases: self.output })
    }
}
