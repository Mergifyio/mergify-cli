...
async def smart_rebase(
    ...
    *,
    ...
    This is the core sync algorithm used by both `stack push` and `stack sync`.
    It checks which PRs have been merged, then does a single `git rebase -i`
    that both drops merged commits and rebases onto the latest trunk.

    If no commits are merged, falls back to a simple `git pull --rebase`.
