...
def _build_stacks(prs: list[DashboardPR]) -> list[list[DashboardPR]]:
    ...
    for members in components.values():
        ...
        bottoms = [
            m for m in members if m not in parent_of or parent_of[m] not in member_set
        ]
...
def _display_section(section: DashboardSection) -> None:
    ...
    for repo, stacks in section.stacks_by_repo.items():
        ...
        for stack_idx, stack in enumerate(stacks):
            ...
            stack_connector = "└──" if is_last_stack else "├──"
            stack_continuation = "    " if is_last_stack else "│   "

            if len(stack) == 1:
                pr = stack[0]
                console.print(
                    f"    {stack_connector} #{pr.number} {pr.title}  "
                    f"[link={pr.url}]{pr.url}[/link]",
                )
            else:
                # First PR in stack is the root of this branch
                first = stack[0]
                console.print(
                    f"    {stack_connector} #{first.number} {first.title}  "
                    f"[link={first.url}]{first.url}[/link]",
                )
                for i, pr in enumerate(stack[1:]):
                    is_last_pr = i == len(stack) - 2
                    pr_connector = "└──" if is_last_pr else "├──"
                    console.print(
                        f"    {stack_continuation} {pr_connector} #{pr.number} {pr.title}  "
                        f"[link={pr.url}]{pr.url}[/link]",
                    )
