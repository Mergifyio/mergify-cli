...
def _count_conditions(
    ...
    if not subconditions:
        return (1, 1) if evaluation["match"] else (0, 1)
    met = 0
    total = 0
    for sub in subconditions:
        s_met, s_total = _count_conditions(sub)
        met += s_met
        total += s_total
    return met, total


def _collect_failing_conditions(
    evaluation: queue_api.QueueConditionEvaluation,
) -> list[str]:
    subconditions = evaluation.get("subconditions") or []
    if not subconditions:
    ...
    for sub in subconditions:
        if not sub["match"]:
            ...
            if child_subs:
                failing.extend(_collect_failing_conditions(sub))
            else:
                failing.append(sub["label"])
    return failing
...
def _print_conditions_section(
    ...
    *,
    ...
    if verbose:
        ...
        met, total = _count_conditions(evaluation)
        ...
        failing = _collect_failing_conditions(evaluation)
