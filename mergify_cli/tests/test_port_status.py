...
def _walk_commands(
    ...
    """Yield the path of every leaf command reachable from ``cmd``.
...
def test_port_status_entries_have_exactly_path_and_status_keys() -> None:
    ...
    for entry in _load_port_status():
        ...
        assert not extras, (
            f"PORT_STATUS.toml entry {entry!r} has unexpected keys: "
            f"{sorted(extras)}. Only {sorted(allowed)} are allowed."
        )
