...
# String-form patch targets so mypy's strict `attr-defined` check
# doesn't reject access to the `junit` / `subprocess` module
# attributes on `upload` (they're imports, not explicit exports).
_RESOLVE_BIN = "mergify_cli.ci.junit_processing.upload.junit._resolve_mergify_binary"
_SUBPROCESS_RUN = "mergify_cli.ci.junit_processing.upload.subprocess.run"
...
def test_upload_invokes_subcommand_with_all_metadata() -> None:
    with (
        mock.patch(_RESOLVE_BIN, return_value="/bin/mergify"),
        mock.patch(_SUBPROCESS_RUN, return_value=_completed()) as run_mock,
    ):
        upload.upload(
...
def test_upload_short_circuits_on_empty_files() -> None:
    ...
    with mock.patch(_SUBPROCESS_RUN) as run_mock:
        upload.upload(
            api_url="https://api.mergify.com",
            token="secret",
            repository="user/repo",
